...
RSpec.shared_context "rake" do
  ...
  def rake_require_file(file_path, loaded_files)
    Rake.application.rake_require(file_path, [Hanami.app.root.to_s], loaded_files)
  end
  ...
  before do
    ...
    loaded_files = $LOADED_FEATURES.reject do |file_path|
      [Hanami.app.root.join("#{task_path}.rake").to_s,
       Hanami.app.root.join("#{task_alternative_path}.rake").to_s].include?(file_path)
    end
